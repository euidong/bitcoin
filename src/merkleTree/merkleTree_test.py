...
from helper.helper import hash256, merkle_root

from src.merkleTree.merkleTree import MerkleTree
...
class MerkleTreeTest(TestCase):
    ...
    def test_populate_tree_3(self):
        hashes = [hash256(int.to_bytes(i, 1, 'big')) for i in range(8)]
        target = hashes[5]
        need_hashes = [
            merkle_root(hashes[:4]),
            hashes[4], hashes[5],
            merkle_root(hashes[6:])
        ]
        flag_bits = [1, 0, 1, 1, 0, 1, 0]
        m_r = merkle_root(hashes)

        m_t = MerkleTree(8)

        self.assertEqual(m_r, m_t.populate_tree(flag_bits, need_hashes))
